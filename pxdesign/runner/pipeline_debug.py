...
def run_live(cmd):
    ...
    while True:
        ...
        if output == '' and process.poll() is not None:
            break
        if output:
            print(output.strip())  # Live print to your program
            
    return process.poll()

def main(argv=None):
    print("HK: getting params from server...")
    url1 = "http://aind-proteomics.corp.alleninstitute.org:5051/api"
    url2 = "http://aind-proteomics.tail2acc6.ts.net:5051/api"
    url2 = "http://100.114.7.72:5051/api" # bare machines do NOT have tailscale but ARE connected so need to provide TS IP which is static
    try:
        r = requests.get(f"{url2}/jobs") # find this job
        r.raise_for_status()
        API_BASE = url2
    except:
        r = requests.get(f"{url1}/jobs") # find this job
        r.raise_for_status()
        API_BASE = url1
    #debug
    # job = next((j for j in r.json() if j["job_id"] == sys.argv[2]), None)
    # if job is None:
    #     print('no jobs matching ID')
    #     sys.exit(1)

    # r = requests.patch(f"{API_BASE}/jobs/{job['id']}/update", json={"status": "Running",}) ## set job to running to get a run assignment
    # r.raise_for_status()
    # job = r.json()
    # if job['run_id'] is None:
    #     r = requests.patch(f"{API_BASE}/jobs/{job['id']}/update", json={ # no job assigned
    #         "status": "Finished", ## assigned job as finished (fine?)
    #     })
    #     r.raise_for_status()
    #     print('no runs to assign')
    #     sys.exit()

    # run_id = job['run_id']
    #debug
    run_id=299
    r = requests.get(f"{API_BASE}/runs/{run_id}") # find this run
    r.raise_for_status()
    run = r.json()
    print(run)
    print(f"Run ID: {run_id}")

    cli = run['settings']['file_path']
    r = requests.get(f"{API_BASE}/uploads/"+cli)
    r.raise_for_status()
    cli = r.json()

    config = run['job_file']
    r = requests.get(f"{API_BASE}/uploads/"+config)
    r.raise_for_status()
    config = r.text

    r = requests.get(f"{API_BASE}/uploads/"+config.split("\n")[1].strip().split('/')[-1])
    r.raise_for_status()
    with open(config.split("\n")[1].strip().split(" ")[-1], 'wb') as f: # built-in full name/hardcoded path
        f.write(r.content)

    # gpu = f"{jax.devices()[0].device_kind}{1}"
    gpu = torch.cuda.get_device_name(0)
#debug
    # r = requests.patch(f"{API_BASE}/jobs/{job['id']}/update", json={ # update job data
    #     "status": "Running",
    #     "gpu_type": gpu,
    #     "hostname": sys.argv[3], # fed in
    #     "run_id": run_id
    # })
    r.raise_for_status()
    # many of these arguments are set by default earlier in the code, so I just copied the args from a pdl1 example and and overwrite the relevant ones
    argv=['--dump_dir', './examples/test_run', '--input_json_path', './examples/PDL1_quick_start.yaml', '--dtype', 'bf16', '--N_sample', '10', '--N_step', '400', '--eta_type', 'const', '--eta_min', '2.5', '--eta_max', '2.5', '--N_max_runs', '1', '--eval.binder.eval_complex', 'true', '--eval.binder.eval_binder_monomer', 'true', '--eval.binder.eval_protenix', 'true', '--eval.binder.eval_protenix_mini', 'false', '--min_total_return', '10', '--max_success_return', '10', ' ', ' ', ' ', ' ', ' ', ' ', '--use_fast_ln', 'True', ' ', '--use_deepspeed_evo_attention', 'True', ' ', '--load_checkpoint_dir', '/home/hari.koneru/PXDesign/release_data/checkpoint']
    ...
    config=config[:config.strip().rindex("\n")]+"\nbinder_length: "+str(cli['binder_length'])
    print(config)
    print(cli)
    #debug
    with open('/PXDesign/config.yaml', 'w') as f:
        f.write(config) # save modified config with out_dir removed (since it's an override) and proper JSON formatting
    ...
    # for i in range(p["N_max_runs"]): # HK treat this as the main design loop, where each iteration corresponds to a round of design -> eval -> selection
    
    trajectory_start_time = time.time()
    i=0
    while True:
        r = requests.patch(f"{API_BASE}/runs/{run_id}/progress", json={
            "minutes_elapsed": (time.time() - trajectory_start_time)/60,
        })
        r.raise_for_status()
        trajectory_start_time = time.time()
        
        ### check if we have to stop
        r = requests.get(f"{API_BASE}/runs/{run_id}/progress")
        r.raise_for_status()
        if r.json()['current_iterations'] > r.json()['target_iterations'] or r.json()['current_binders'] > r.json()['target_binders'] or r.json()['minutes_elapsed'] > r.json()['max_time_minutes']:
        ...
        local_seed = derive_seed(time.time_ns() % (2**31 - 1), DIST_WRAPPER.rank)
        ...
        with open(all_eval_results[0]["sample_save_path"], 'r') as file:
            reader = csv.DictReader(file)
            for row in reader:
                af2=row['af2_opt_success']
                ptx=row['ptx_success']
                seq=row['sequence']
                idx=row['name'].split("_")[-1]
                r = requests.patch(f"{API_BASE}/runs/{run_id}/progress", json={
                    "current_iterations": 1,
                })
                r.raise_for_status()
                if int(af2)==1 and int(ptx)==1:
                    r = requests.patch(f"{API_BASE}/runs/{run_id}/progress", json={
                        "current_binders": 1,
                    })
                    r.raise_for_status()
                    r = requests.post(f"{API_BASE}/runs/{run_id}/binders", json={
                        "binder_id": argv[1].split("/")[-1]+"_"+str(local_seed)+"_"+str(idx),
                        "binder_sequence": seq,
                    })
        r.raise_for_status()
