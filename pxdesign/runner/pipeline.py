...
def main(argv=None):
    ...
    r = requests.patch(
        f"{API_BASE}/jobs/{job['id']}/update",
        json={
            "status": "Running",
        },
    )  ## set job to running to get a run assignment
    r.raise_for_status()
    job = r.json()
    if job["run_id"] is None:
        r = requests.patch(
            f"{API_BASE}/jobs/{job['id']}/update",
            json={  # no job assigned
    ...
    r = requests.get(
        f"{API_BASE}/uploads/structures/" + config.split("\n")[1].strip().split("/")[-1]
    )
    r.raise_for_status()
    with open(
        config.split("\n")[1].strip().split(" ")[-1], "wb"
    ) as f:  # built-in full name/hardcoded path
        f.write(r.content)
    ...
    config = (
        config[: config.strip().rindex("\n")]
        + "\nbinder_length: "
        + str(cli["binder_length"])
    )
    print(config)
    print(cli)
    ...
    while True:
        r = requests.patch(
            f"{API_BASE}/runs/{run_id}/progress",
            json={
        ...
        r.raise_for_status()
        if (
            r.json()["current_iterations"] > r.json()["target_iterations"]
            or r.json()["current_binders"] > r.json()["target_binders"]
            or r.json()["minutes_elapsed"] > r.json()["max_time_minutes"]
        ):
        ...
        with open(all_eval_results[0]["sample_save_path"], "r") as file:
            ...
            for row in reader:
                ...
                if int(af2) == 1 and int(ptx) == 1:
                    ...
                    r.raise_for_status()
                    r = requests.post(
                        f"{API_BASE}/runs/{run_id}/binders",
                        json={
                            "binder_id": argv[1].split("/")[-1]
                            + "_"
                            + str(local_seed)
                            + "_"
                            + str(idx),
