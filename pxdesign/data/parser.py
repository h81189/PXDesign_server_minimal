...
from pxdesign.data.utils import (
    atom_select,
...
class AddAtomArrayAnnot(object):
    ...
    def add_distogram_rep_atom_mask(atom_array: AtomArray) -> AtomArray:
        ...
        pbb = atom_array.res_name == "xpb"

        prot_cb = std_prot_res & (~gly) & (~pbb) & (atom_array.atom_name == "CB")
        prot_gly_ca = gly & (atom_array.atom_name == "CA")
        prot_pbb_ca = pbb & (atom_array.atom_name == "CA")
        ...
        distogram_rep_atom_mask = (
            prot_cb | prot_gly_ca | prot_pbb_ca | purines_c4 | pyrimidines_c2 | unk_nuc
        ) | (~std_res)
        ...
        if np.sum(atom_array.distogram_rep_atom_mask) != np.sum(
            ...
            for start, stop in zip(starts[:-1], starts[1:]):
                if ~np.any(atom_array.distogram_rep_atom_mask[start:stop]):
                    logger.warning(
                        f"This residue has no distogram_rep_atom, \
                            use the first atom: \
                            res_chain:{atom_array.chain_id[start]} \
                                res_id: {atom_array.res_id[start]}, \
                                    res_name: {atom_array.res_name[start]}"
                    )
    ...
    def unique_chain_and_add_ids(atom_array: AtomArray) -> AtomArray:
        ...
        for i in range(len(start_indices) - 1):
            ...
            ori_chain_id = atom_array.chain_id[start_i]
            if any(char.isdigit() for char in ori_chain_id):
                new_chain_id = ori_chain_id
            else:
                new_chain_id = f"{ori_chain_id}{sym_ids[start_i]}"
