...
from pxdesign.data.tokenizer import Token, TokenArray
from pxdesign.utils.design import make_random_mask
...
class DesignFeaturizer(object):
    ...
    def get_condition_template_feature(
        ...
        if templ_token_mask is not None:
            condi_token_rslv_mask *= templ_token_mask.astype(bool)
        if not condi_token_rslv_mask.any():
            return feature_dict

        # Compute distogram bins as the conditional template
        ...
        boundaries = torch.linspace(start=2.0, end=22.0, steps=64 - 1)
        condi_templ_bins = torch.sum(
            condi_token_dist.unsqueeze(dim=-1) > boundaries, dim=-1
        )
        idx = torch.nonzero(torch.tensor(condi_token_rslv_mask).long(), as_tuple=True)[
            0
        ]
        ii, jj = torch.meshgrid(idx, idx, indexing="ij")
        assert condi_templ_bins.shape == (len(idx), len(idx))
        ...
        feature_dict["conditional_templ_mask"] = conditional_templ_mask
