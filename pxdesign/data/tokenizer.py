...
from pxdesign.data.constants import ELEMS, STD_RESIDUES
...
class TokenArray(object):
    ...
    def set_annotation(self, category, values):
        assert len(values) == len(
            self.tokens
        ), f"Length of {len(values)=} must match the number of {len(self.tokens)=}"
    ...
    def get_token_array(self) -> TokenArray:
        ...
        Returns:
            TokenArray: The TokenArray object with annotations.
                ...
                Token($token_index,  atom_indices=[global_atom_indexs], centre_atom_index=global_atom_indexs,
                   atom_names=[names])
        """
